...
-- 7. Find the largest country (by area) in each continent, show the continent, 
	...
	--A correlated subquery works like a nested loop: the subquery only has access to rows related
	--to a single record at a time in the outer query. The technique relies on table aliases to 
	--identify two different uses of the same table, one in the outer query and the other in the subquery.
	--One way to interpret the line in the WHERE clause that references the two table is “… where the correlated 
	--values are the same”.
